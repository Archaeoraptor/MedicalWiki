...
//初始化SEO
func initSeo() {
	sqlslice := []string{"insert ignore into `md_seo`(`id`,`page`,`statement`,`title`,`keywords`,`description`) values ('1','index','发现','医学知识库，发现更多知识','{keywords}','{description}'),",
		"('2','label_list','标签列表页','{title} - 医学知识库','{keywords}','{description}'),",
		"('3','label_content','标签内容页','{title} - 医学知识库','{keywords}','{description}'),",
		"('4','book_info','文档信息页','{title} - 医学知识库','{keywords}','{description}'),",
		"('5','book_read','文档阅读页','{title} - 医学知识库','{keywords}','{description}'),",
		"('6','search_result','搜索结果页','{title} - 医学知识库','{keywords}','{description}'),",
		"('7','user_basic','用户基本信息设置页','{title}  - 医学知识库','{keywords}','{description}'),",
		"('8','user_pwd','用户修改密码页','{title}  - 医学知识库','{keywords}','{description}'),",
		"('9','project_list','书籍列表页','{title}  - 医学知识库','{keywords}','{description}'),",
		"('11','login','登录页','{title} - 医学知识库','{keywords}','{description}'),",
		"('12','reg','注册页','{title} - 医学知识库','{keywords}','{description}'),",
		"('13','findpwd','找回密码','{title} - 医学知识库','{keywords}','{description}'),",
		...
		"('22','ucenter-fans','用户粉丝','{title} - 医学知识库','{keywords}','{description}'),",
		"('23','ucenter-follow','用户关注','{title} - 医学知识库','{keywords}','{description}');",
	}
